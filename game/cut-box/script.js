...
// Colors
const BLUE =   { r: 0x67, g: 0xd7, b: 0xf0 }, GREEN =  { r: 0xa6, g: 0xe0, b: 0x2c }, PINK =   { r: 0xfa, g: 0x24, b: 0x73 }, ORANGE = { r: 0xfe, g: 0x95, b: 0x22 }, allColors = [BLUE, GREEN, PINK, ORANGE];

// Gameplay
const getSpawnDelay = () => { 
    const spawnDelayMax = 1400, spawnDelayMin = 550, spawnDelay = spawnDelayMax - state.game.cubeCount * 3.1;
	return Math.max(spawnDelay, spawnDelayMin);
}
const doubleStrongEnableScore = 2000, slowmoThreshold = 10, strongThreshold = 25, spinnerThreshold = 25

let pointerIsDown = false, pointerScreen = { x: 0, y: 0 }, pointerScene = { x: 0, y: 0 };

const minPointerSpeed = 60, hitDampening = 0.1, backboardZ = -400, shadowColor = '#262e36', airDrag = 0.022, gravity = 0.3, sparkColor = 'rgba(170,221,255,.9)', sparkThickness = 2.2, airDragSpark = 0.1, touchTrailColor = 'rgba(170,221,255,.62)', touchTrailThickness = 7, touchPointLife = 120, touchPoints = [], targetRadius = 40, targetHitRadius = 50;
const makeTargetGlueColor = target => {
	return 'rgb(170,221,255)';
};

const fragRadius = targetRadius / 3, canvas = document.querySelector('#c'), cameraDistance = 900, sceneScale = 1, cameraFadeStartZ = 0.45*cameraDistance, cameraFadeEndZ = 0.65*cameraDistance, cameraFadeRange = cameraFadeEndZ - cameraFadeStartZ, allVertices = [], allPolys = [], allShadowVertices = [], allShadowPolys = [], GAME_MODE_RANKED = Symbol('GAME_MODE_RANKED'), GAME_MODE_CASUAL = Symbol('GAME_MODE_CASUAL'), MENU_MAIN = Symbol('MENU_MAIN'), MENU_PAUSE = Symbol('MENU_PAUSE'), MENU_SCORE = Symbol('MENU_SCORE');
...
const isInGame = () => !state.menus.active, isMenuVisible = () => !!state.menus.active, isCasualGame = () => state.game.mode === GAME_MODE_CASUAL, isPaused = () => state.menus.active === MENU_PAUSE, highScoreKey = '__menja__highScore';
const getHighScore = () => {
